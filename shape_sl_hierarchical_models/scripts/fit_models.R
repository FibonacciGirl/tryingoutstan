...
fit.vec.7 <- stan(file = 'rt_constant-piecewiseRLR.stan', data = model.data.1, iter = 10000,warmup =1000 , 
            chains = 1, verbose = T)

pairs(fit.vec.9)

plot(fit.vec.7,par= c('log_q_z1[1]','log_q_z2[1]'))
plot(fit.vec.7,par= c('pi0'))
plot(fit.vec.7,par= c('sigma_mean'))
model
...
select<-c()
for(j in 1:J){
  if(model[[j]]=='power.logistic'){this=1}
  else{this=0}
