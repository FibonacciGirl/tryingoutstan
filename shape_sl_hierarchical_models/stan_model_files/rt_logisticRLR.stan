...
parameters{
  ...
  vector <lower = 0>[J] rlr_proportion;
  vector <lower = 0>[J] rlr_proportion2;
  vector <lower = 0>[J] rlr_rate;
  vector <lower = 0>[J] rlr_split;
  ...
  real <lower = 0>  rlr_proportion_mean;
  real <lower = 0>  rlr_proportion_var;
  real <lower = 0>  rlr_proportion2_mean;
  real <lower = 0>  rlr_proportion2_var;
  real <lower = 0>  rlr_rate_mean;
  real <lower = 0>  rlr_rate_var;
  real <lower = 0>  rlr_split_mean;
  real <lower = 0>  rlr_split_var;
}
...
transformed parameters{
  ...
  for(j in 1:J){
    for(t in 1:Tr){
      ...
      rlr[j,t]= rlr_proportion[j]+((rlr_proportion[j]*rlr_proportion2[j]-rlr_proportion[j])/(1+exp(rlr_rate[j]*(t-rlr_split[j]))));
...
model{
  ...
  target+= gamma_lpdf(rlr_proportion_mean|mu1,tau1);
  target+= gamma_lpdf(rlr_proportion_var|a3,b3);
  target+= gamma_lpdf(rlr_proportion2_mean|mu2,tau2);
  target+= gamma_lpdf(rlr_proportion2_var|a4,b4);
  target+= gamma_lpdf(rlr_split_mean|alpha2,beta2);
  target+= gamma_lpdf(rlr_rate_mean|alpha3,beta3);
  target+= gamma_lpdf(rlr_split_var|a5,b5);
  ...
  target += normal_lpdf(rlr_proportion| rlr_proportion_mean, rlr_proportion_var);
  target += normal_lpdf(rlr_proportion2| rlr_proportion2_mean, rlr_proportion2_var);
  target += normal_lpdf(rlr_split| rlr_split_mean, rlr_split_var);
